...
export function ReduceIntensityDialog({
  alert,
  onClose,
}: {
  alert: CoachAlert;
  ...
  const submit = async () => {
    ...
    if (Object.keys(payload).length === 0) {
      toast.error('Definí al menos un ajuste');
      return;
    }
    try {
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            Bajar intensidad · {alert.exercise_name ?? 'ejercicio'}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-2 text-sm">
          <label className="block">
            Δ series (negativo, ej. -1)
            <Input
              type="number"
              max={0}
              min={-5}
          ...
          <label className="block">
            ...
            <Input
              ...
              value={rpeDelta}
              onChange={(e) =>
                setRpeDelta(e.target.value === '' ? '' : Number(e.target.value))
              }
            />
          </label>
          <Textarea
