...
export function ApproveSwitchDialog({
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Aprobar cambio del atleta</DialogTitle>
        </DialogHeader>
        <p className="text-sm">
          Atleta cambió <strong>{alert.exercise_name ?? 'ejercicio'}</strong>{' '}
          por <strong>ejercicio ID {p.switched_to_exercise_id ?? '?'}</strong>.
