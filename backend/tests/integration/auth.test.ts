...
const { signupUserInDb, verifiedAthleteUser } =
...
it('signup remains compatible with clients that do not send contact fields', async () => {
...
it('signup rejects an incomplete contact group', async () => {
  const r = await request(app).post('/api/auth/signup').send({
...
it('login blocked when email not verified', async () => {
  const { email, password } = {
    email: 'unver@test.local',
    password: 'pwd-test-1234',
  };
...
it('verified-but-unverified-email takes precedence over status gate', async () => {
  ...
  const { email, password } = {
    email: 'unver-pending@test.local',
    password: 'pwd-test-1234',
  };
...
it('verify-email marks user verified, second click 400 used', async () => {
  const { id, verifyToken } = await signupUserInDb(
    'vmark@test.local',
    'pwd-test-1234',
    false
  );
  ...
  const u = await pool.query(`SELECT email_verified FROM users WHERE id = $1`, [
    id,
  ]);
...
async function seedKnownCode(userId: string, code: string): Promise<void> {
  ...
  const { expiresIn, RESET_TOKEN_TTL_MS } =
