...
const sendExpiring = jest
  .fn<(o: never) => Promise<void>>()
  .mockResolvedValue(undefined);
const sendExpired = jest
  .fn<(o: never) => Promise<void>>()
  .mockResolvedValue(undefined);
jest.unstable_mockModule('../../src/services/email.service.js', () => ({
  ...
  sendCommunityRevisionEmail: async () => {},
}));

const { resetDatabase, ensureMigrated, closePool } =
  await import('./helpers/test-db.js');
const { createAdmin, createAthlete, setMembership } =
  await import('./helpers/fixtures.js');
const poolMod = await import('../../src/db/connect.js');
const pool = poolMod.default;
const { runMembershipTick } =
  await import('../../src/workers/membership-cron.js');
