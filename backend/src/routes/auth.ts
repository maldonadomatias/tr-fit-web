...
router.delete(
  '/account',
  requireAuth,
  skipInTests(deleteAccountLimiter),
  async (req: Request, res: Response) => {
...
router.get('/verify-email', async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(200)
      .type('html')
      .send(verifySuccessPage(env.APP_DEEP_LINK_SCHEME));
  } catch (e) {
...
router.post(
  '/resend-verification',
  requireAuth,
  skipInTests(resendVerifyLimiter),
  async (req: Request, res: Response) => {
...
router.post(
  '/forgot-password',
  skipInTests(forgotPasswordLimiter),
  async (req: Request, res: Response) => {
